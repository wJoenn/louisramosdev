---
  ...
  description: How to set up a Vue client with client side state management for our Rails API and authentication
...
And the second one will be used to run both our Rails server and our Vite server from a single command inside a single terminal. Let's call it `bin/dev`
```bash [bin/dev]
#!/bin/bash
...
<a href="https://github.com/open-cli-tools/concurrently" target="_blank">Concurrently</a> is a tool that'll allow us to run both server <em>concurrently</em> and within the same terminal.

Let's make them executable by modifying their rights.
```bash
chmod +x bin/vite
chmod +x bin/dev
```

And you can now install the dependencies for your client app then run both servers.
```bash
bin/vite install
bin/dev
```

Once you've seen it works you can remove all the boilerplate code from `create-vue` as we'll write our own from now on.

## Handling user authentication with State Management

The first thing that'll be asked from us when using the app is to log in and because of that the first thing we need to focus on is to write the logic to authenticate users and persist their sessions.

`State Management` in Vue is done with <a href="https://pinia.vuejs.org/" target="_blank">Pinia</a> like I mentionned before. It was installed during the creation of Vue app so we can just create a new Store `client/src/stores/UserStore.js`.
...
As you can see we need to define some `state`, `getters` and `actions`.

The `state` is the central part of the store so let's start there. It defines the variables that will be added to the State of our application and in our case that would be a user Object and the JSON Web Token linked to this user and that's it.
```js [client/src/stores/UserStore.js]
...
...
The `getters` are readonly variables computed from our state variables. You might or might now need one in your stores, in the context of this UserStore it'd be nice to have a computed value of whether a user is logged in or not so that's what we'll make.
```js [client/src/stores/UserStore.js]
...
...
Any change you want to apply to the state of a Store is best done through an action. Let's make a couple to register, sign in and sign out.
```js [client/src/stores/UserStore.js]
...
...
As you can see in the `postRequest` method, everytime a user sucessfully login we save his Bearer token inside its browser's `localeStorage`. This will allow us to attempt to log in the user everytime he revisit the website later on.

To do that let's go inside our main js file, in Vue that is `src/main.js`, and just before we mount the app to the DOM let's use our `loginWithToken` method.
...
A lot of things happened in the last couple code blocks but basically what happened is I created a login and a signup form which do not require a user to be authenticate to be accessed (obviously) and I created a homepage which we will use later to display our to-do list and because tasks require a user added an auth guard to this route so that a visitor would be prompted to log in if he tries to access this page.
...
All we have left to do is add a form that will send a POST request to our API to create a new task, display all our existing tasks and give each of them a button to complete and a button to destroy.
...
<script setup>
  ...
  const { bearerToken } = toRefs(useUserStore())
  const tasks = ref([])
  ...
  const handleFormSubmit = async event => {
    ...
    try {
      const response = await fetch(`${API_URL}/tasks`, {
        method: "POST",
        body: JSON.stringify({ task: Object.fromEntries(formData) }),
        headers: {
...
Congratulation on following along this serie of articles ! REST APIs are a very common way to build applications nowadays and from now on you can practice this with confidence.
